...
class Google_Custom_Search extends WP_Widget {
	...
	private function create_search_results_template() {
		...
		if( null == get_page_by_title( 'search results' ) ) {
			...
			// Get the content
			$page_content = wp_remote_get( get_template_directory_uri() . '/lib/google-custom-search/lib/Google_Custom_Search.template.php' );
			...
			$page_id = wp_insert_post(
				array(
					...
					'post_title'		=>	__( 'Search Results', TRANSLATION_KEY ),
					'post_status'		=>	'publish',
					'post_type'			=>	'page',
			...
			if ( 0 == count( get_post_meta( $page_id, 'google_custom_search' ) ) ) {
				delete_post_meta( $page_id, 'google_custom_search' );
			} // end if

			// Mark that this post was created by theme
			update_post_meta( $page_id, 'google_custom_search', true );
	...
	public function delete_search_results_template() {
		...
		$query = new WP_Query('post_type=page&meta_key=google_custom_search');
		if( $query->have_posts() ) {
	...
	public function existing_search_results_template() {
		...
		if( $this->is_page( 'widgets.php' ) ) {
			...
			if( 1 != get_post_meta( $page_id, 'google_custom_search', true ) ) {

				echo '<div id="gcse-template-exists-notification" class="updated"><p>' . __( 'Could not configure Google Custom Search widget because the required "search-results" permalink is already in use. Please rename <a href="post.php?post=' . $page_id . '&action=edit">the existing page\'s permalink</a>, or <a href="post.php?post=' . $page_id . '&action=edit">delete the page</a>, and try again.', TRANSLATION_KEY) . '</p></div>';
		...
		// Otherwise, if we're editing the generated Search Results page, then we need to display a message.
			...
			// Otherwise, they are on the post edit page so they should be able to see the message.
				...
				echo '<div id="gcse-template-notification" class="updated"><p>' . __( 'This page was generated by Lean for use with the Google Custom Search widget. <a href="widgets.php">Remove the widget</a> from all sidebars to delete this page.', TRANSLATION_KEY ) . '</p></div>';
